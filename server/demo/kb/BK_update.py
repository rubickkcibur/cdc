...
def start(basic_cliques,build=False,traverse=False,dmax=5,version=2):
    global g,all_max_cliques,location_size
    if build:
        g = build_graph(dmax)
        time_start = time.time()
        for p in basic_cliques: 
            if version == 1:
                Bron_Kerbosch(set(),set(p),set())
            elif version == 2:
                Bron_Kerbosch2(set(),set(p),set())
            elif version == 3:
                label_sets = set()
                for i in range(len(p)):
                    for j in range(i+1,len(p)):
                        if g[p[i],p[j]] != 0:
                            label_sets.add((1 << p[i]) | (1 << p[j]))
                Dynamic(label_sets)
        print("search done,time:",time_start-time.time())
        with open("all_max_cliques_v{}_update_{}.json".format(version,dmax),"w",encoding="utf-8") as f:
            json.dump(all_max_cliques,f,ensure_ascii=False)
    else:
        all_max_cliques = json.load(open("all_max_cliques_v{}_update_{}.json".format(version,dmax),"r",encoding="utf-8"))
    ...
    if traverse:
        ...
        with open("patient_cliques_v{}_update_{}.json".format(version,dmax),"w",encoding="utf-8") as f:
            json.dump(patient_cliques,f,ensure_ascii=False)
    else:
        patient_cliques = json.load(open("patient_cliques_v{}_update_{}.json".format(version,dmax),"r",encoding="utf8"))
...
old_max_cliques = json.load(open("./all_max_cliques_v2_3.5.json","r",encoding="utf8"))
clique_no = json.load(open("./clique_no_v2_3.5.json","r",encoding="utf8"))
basic_cliques = [old_max_cliques[i] for i in clique_no]
location_size = len(no2name)
_,patc = start(basic_cliques=basic_cliques,build=True,traverse=True,version=BK_VERSION,dmax=DMAX)
result,clique_no = minimize_patient_cliques(patc)
with open("./result_v{}_update_{}.json".format(BK_VERSION,DMAX),"w",encoding="utf8") as fr,\
    open("./clique_no_v{}_update_{}.json".format(BK_VERSION,DMAX),"w",encoding="utf8") as fc:
